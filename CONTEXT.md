...
### Remise % persistence
Per-card `Remise %` is persisted in `data/remises.json` (keyed by `card_slug`).
Loaded on app start, saved on any change. Lost on Streamlit Cloud if the repo is not updated —
commit `data/remises.json` after manual edits in the app.

## Tab 6 — Équipe & Tab 8 — Mes Lineups : mini histogrammes

Both tabs display a sparkline bar chart of the last 5 match scores under each card image.

- Rendered as inline SVG via `st.markdown(unsafe_allow_html=True)` — **not** a Plotly/Altair chart.
- Generated by `gen_bar_sparkline_svg(scores, w, h, score_colors=True)` in `data_loaders.py`.
- Data sourced from `load_last5_scores(slugs: tuple) -> dict[slug, list]` — last 5 rows of
  `game_scores.parquet` per player, **no** `played_in_game` filter.
- DNP (not played) → `None` in the list → rendered as a small grey bar (`rgb(100,116,139)`, height h/4).
- Score colour thresholds (applied when `score_colors=True`):

  | Score | Colour |
  |-------|--------|
  | ≤ 2   | `rgb(255,90,90)` — red |
  | ≤ 7   | `rgb(255,126,52)` — orange |
  | ≤ 11  | `rgb(240,206,29)` — yellow |
  | ≤ 15  | `rgb(182,255,26)` — yellow-green |
  | ≤ 30  | `rgb(37,237,54)` — green |
  | ≤ 45  | `rgb(0,243,235)` — cyan |
  | > 45  | `rgb(223,227,244)` — white/light |

## Tab 5 — Projections GW

- "Sorare proj." column sourced from `load_sorare_projections()` in `data_loaders.py`.
- Reads `players.parquet` which now includes `next_gw_projected_score` (exported by `update_data.py`
  step 11 from `mlb.players.next_gw_projected_score`).
- Available for **all** players in `players.parquet` (not just gallery), so the
  "Charger les données de tous les joueurs" checkbox shows Sorare projections for the full league.
...
- **Leaderboard data fetched mid-competition is partial** — `fetch_leaderboard_history.py` stores
  `fromSo5Ranking.overallScore` as `score_threshold`. If fetched while a GW is still open, scores
  are in-progress (e.g. GW138 top showed 136 instead of 368). Fix: strip the bad GWs from
  `data/leaderboard_rewards.parquet` and re-run `python update_data.py --only 10`.
