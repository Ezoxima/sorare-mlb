...
_RAR_DOT = {
    "Limited":   "var(--r-limited)",
    "Rare":      "var(--r-rare)",
    "Super Rare":"var(--r-superrare)",
    "Unique":    "var(--r-unique)",
}
_RAR_BORDER = {
    "limited":    "var(--r-limited)",
    "rare":       "var(--r-rare)",
    "super_rare": "var(--r-superrare)",
    "unique":     "var(--r-unique)",
}
_SCORE_STATUS_COLOR = {
    "DONE": "var(--pos)", "COMPLETED": "var(--pos)",
    "REVIEWING": "var(--warn)", "LIVE": "var(--warn)",
...
def render(ctx: dict) -> None:
    ...
    def _rar_fmt(x: str) -> str:
        col = _RAR_DOT.get(x, "var(--fg-3)")
        return f'<span style="display:inline-flex;align-items:center;gap:5px"><span style="width:7px;height:7px;border-radius:50%;background:{col};display:inline-block;box-shadow:0 0 5px -1px {col}"></span>{x}</span>'

    # ── Zone filtres défis (même style que le prototype HTML) ──────────────────
    with st.container(border=True):
        # En-tête de la zone
        st.markdown(
            f'<div style="display:flex;align-items:center;justify-content:space-between;'
            f'padding:2px 0 10px;border-bottom:1px solid var(--line);margin-bottom:12px">'
            f'<div style="display:flex;align-items:center;gap:7px">'
            f'<span style="color:var(--fg-3);font-size:11px">🏆</span>'
            f'<span style="font:700 10px/1 var(--mono);letter-spacing:.1em;'
            f'text-transform:uppercase;color:var(--fg-1)">DÉFIS JOURNALIERS</span>'
            f'</div>'
            f'<span style="font:600 9px/1 var(--mono);letter-spacing:.1em;'
            f'text-transform:uppercase;color:var(--fg-3)">{_tab1_day_label.upper()}</span>'
            f'</div>',
        ...
        if categorie == "HITTING":
            rar_c, s1, sai_c, s2, pos_c, s3, ten_c, s4, tri_c = st.columns(
                [2.7, 0.08, 1.0, 0.08, 1.3, 0.08, 1.15, 0.08, 1.7]
            )
            _seps = (s1, s2, s3, s4)
        else:
            rar_c, s1, sai_c, s2, pos_c, s3, ten_c = st.columns(
                [2.7, 0.08, 1.0, 0.08, 1.3, 0.08, 1.15]
            )
    ...
    def _t5_match_cell(team_slug: str) -> str:
        ...
        def _lc(sl):
            ...
            return (f'<div style="display:flex;flex-direction:column;align-items:center;gap:2px">'
                    f'{img}<span style="font-size:8px;font-weight:700;color:var(--fg-3);letter-spacing:.04em">{code}</span></div>')
    ...
    if not _df_top5.empty:
        ...
        _hs       = "font-size:8px;letter-spacing:.1em;text-transform:uppercase;color:var(--fg-3);font-family:var(--mono)"
        ...
        _rows_html = (
            ...
            f'padding-bottom:5px;margin-bottom:4px;border-bottom:1px solid var(--line)">'
        ...
        for _t5_rank, (_, _t5_row) in enumerate(_df_top5.iterrows()):
            ...
            _t5_bgs   = ["rgba(247,177,0,0.07)", "rgba(170,180,194,0.05)", "rgba(205,127,50,0.05)"]
        ...
        _t5_mode_pill = (
            f'<span style="font-size:9px;padding:2px 6px;border:1px solid var(--line-2);'
            f'color:var(--fg-2);border-radius:3px">{"vs Pitcher" if _pitcher_mode else "tous joueurs"}</span>'
        )
        _top5_panel_html = (
            ...
            f'color:var(--fg-2);margin-bottom:10px;display:flex;align-items:center;gap:6px;'
            f'font-family:var(--mono)">TOP 5 {_t5_mode_pill}</div>'
            f'{_rows_html}'
            f'</div>'
        )

    # ── Filtre par rareté galerie ──────────────────────────────────────────────
    ...
    # ── Panel principal : Défis du jour + TOP 5 ────────────────────────────────
    _all_show_tasks = [
        t for t in _tasks
        ...
        t for t in _tasks
        if t.get("typename") == "ThresholdsStreakTask"
    ...
    def _slot_html(ap: dict | None, slot_idx: int) -> str:
        ...
        badge = (f'<span style="position:absolute;bottom:-3px;right:-3px;font-size:8px;'
                 f'color:{sc_col};line-height:1">{check}</span>' if check else "")
        return (
    ...
    def _task_html(task: dict) -> str:
        ...
        bc      = _RAR_BORDER.get(_rar_val, "var(--line-2)")
        icon_h  = (f'<img src="{icon}" style="height:13px;width:13px;object-fit:contain;flex-shrink:0">'
                   if icon else "")
        return (
            f'<div style="padding:10px 14px 10px 12px;border-bottom:1px solid var(--line);'
            f'border-left:2px solid {bc}">'
            f'<div style="display:flex;align-items:center;gap:6px;margin-bottom:3px">'
            f'{icon_h}'
            f'<span style="font-size:11px;font-weight:700;color:var(--fg-0)">{title}</span>'
            f'</div>'
            f'<div style="font-size:8px;color:var(--fg-3);white-space:nowrap;overflow:hidden;'
            f'text-overflow:ellipsis">{desc}</div>'
            f'</div>'
        )

    _tasks_html = "".join(_task_html(t) for t in _all_show_tasks) if _all_show_tasks else ""
    _hs_t = "font-size:9px;letter-spacing:.12em;text-transform:uppercase;color:var(--fg-2);font-family:var(--mono)"

    if _tasks_html:
        _tasks_col_html = (
            f'<div class="t1-tasks-col" style="padding:12px 16px;overflow-y:auto;'
            f'border-right:1px solid var(--line)">'
            f'<div style="{_hs_t};margin-bottom:10px">Défis du jour</div>'
            f'{_tasks_html}'
            f'</div>'
        )
        _grid_cols = "1fr 1fr"
    ...
    # Rareté dot pour le panel header
    _rar_dot_col = _RAR_BORDER.get(_rar_val, "var(--fg-3)")
    _rar_dot_html = (
        f'<span style="width:7px;height:7px;border-radius:50%;background:{_rar_dot_col};'
        f'display:inline-block;box-shadow:0 0 5px -1px {_rar_dot_col}"></span>'
    )

    st.markdown(
        f'<div class="panel">'
        f'<div class="panel__hd">'
        f'<span style="font-size:11px;line-height:1">🏆</span>'
        f'<span class="title">Défis journaliers</span>'
        f'<span class="pill accent">{_tab1_day_label}</span>'
        f'{_rar_dot_html}'
        f'<span style="font-size:9px;color:{_rar_dot_col};font-family:var(--mono);letter-spacing:.04em">{_rar_label}</span>'
        f'<span class="right">{fenetre} · {categorie}</span>'
        f'</div>'
        f'<div class="t1-grid" style="display:grid;grid-template-columns:{_grid_cols};'
        f'border-top:1px solid var(--line)">'
        f'{_tasks_col_html}'
        f'<div style="display:flex;min-width:0">{_top5_panel_html}</div>'
        f'</div></div>',
        unsafe_allow_html=True,
    )
    ...
    if not df_prices.empty and sel_manager:
        ...
        _mgr_cards = _mgr_cards.sort_values(["in_season_eligible", "_rar_rank"], ascending=[False, True])
        _top3_pic_map = (
    ...
    for i, (_, row) in enumerate(top3.iterrows()):
        ...
        _sfx = "color:#ffffff;opacity:0.65;font-weight:normal;font-size:.82em"
        if _t3_sea and _t3_ser:
            _t3_suffix = f' <span style="{_sfx}">· {_t3_sea} · #{_t3_ser}</span>'
        elif _t3_ser:
            _t3_suffix = f' <span style="{_sfx}">· #{_t3_ser}</span>'
        elif _t3_sea:
            _t3_suffix = f' <span style="{_sfx}">· {_t3_sea}</span>'
        else:
            _t3_suffix = ""
        _top3_cards_html += (
    ...
    _RAR_COLOR = {
        "limited":    "var(--r-limited)", "rare":      "var(--r-rare)",
...
div[data-testid="stHorizontalBlock"] div[data-testid="stHorizontalBlock"] div[data-testid="stColumn"]:last-of-type button {
  ...
  background:rgba(160,30,30,.13)!important;
  border:1px solid rgba(200,60,60,.28)!important;
  color:rgba(220,70,70,.75)!important;
...
div[data-testid="stHorizontalBlock"] div[data-testid="stHorizontalBlock"] div[data-testid="stColumn"]:last-of-type button:hover {
  background:rgba(200,40,40,.22)!important;
  border-color:rgba(220,70,70,.5)!important;
  color:rgba(240,90,90,.95)!important;
}
...
""", unsafe_allow_html=True)
    ...
    _hdr_style = ("font-family:var(--mono);font-size:9px;letter-spacing:.12em;"
    ...
    if _pitcher_mode:
        _h5.markdown(
            f'<div style="{_hdr_c}" title="Moy. accordée vs ligue ({_pf_league_avg:.2f}/match)">Pitcher</div>',
            unsafe_allow_html=True,
        )
        with _h6:
    ...
    for _, row in _df_expanded.iterrows():
        ...
        _sfx_style   = "color:#ffffff;opacity:0.65;font-weight:normal;letter-spacing:.03em"
        _card_suffix = ""
        if _sea and _ser:
